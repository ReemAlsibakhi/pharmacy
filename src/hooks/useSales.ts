  ...
  const { data, error } = await supabase
    ...
    .select(`
      *,
      ...
      sale_items(*, product:products(name_ar))
    `)
...
async function fetchSaleById(id: string): Promise<Sale> {
  const { data, error } = await supabase
    ...
    .select(`
      *,
      customer:customers(id, name, phone),
      sale_items(*, product:products(name_ar, barcode), batch:batches(batch_number, expiry_date))
    `)
    .eq('id', id)
    .single()
...
async function completeSale(payload: CompleteSalePayload) {
  ...
  if (error) throw new Error(error.message)
  return data?.[0] as { sale_id: string; invoice_number: string }
}
